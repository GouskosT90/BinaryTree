/************************************************
	Name: Theodore Gouskos
	Date: 4/4/18

	Abstract: tree.h - binary tree that holds
	 integers
************************************************/
...
//Post: BFS of a Binary Tree
	...
	while(!myQ.IsEmpty()) //While the que is not empty
	{
		myQ.Dequeue(current);
		cout << current->data << " ";
	...
	if (current-> leftChild != 0)
		{
		myQ.Enqueue(current->leftChild);
		}

	if (current-> rightChild != 0)
		{
		myQ.Enqueue(current->rightChild);
		}
	}
}
